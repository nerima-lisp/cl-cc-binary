...
(define-binary-writer %elf64-write-exec-ehdr
    (out builder entry-point phoff shoff phnum n-sections shstrtab-idx)
  ...
  (:u8 +elf-osabi-none+)
  (:pad 8)
  (:u16 (elf64-elf-type builder))
  (:u16 (elf64-machine builder))
  (:u32 +elf-version-cur+)
  (:u64 entry-point)
  (:u64 phoff)
  (:u64 shoff)
...
(defun %elf64-section-header-indices (dynamic-p interp-p rodata-str-section-delta)
  "Return N-SECTIONS, DYNSYM-IDX, DYNSTR-IDX, SYMTAB-IDX, STRTAB-IDX,
DEBUG-LINE-IDX, and SHSTRTAB-IDX: the section header table indices for an
ELF64 executable, derived from which optional sections (.interp,
.rodata.str, the ET_DYN dynamic-linking group) are present."
  (let* ((n-sections (+ (if dynamic-p (+ 16 (if interp-p 1 0)) 11)
    ...
    (values n-sections dynsym-idx dynstr-idx symtab-idx strtab-idx debug-line-idx shstrtab-idx)))
...
(defun elf64-finalize-executable (builder)
  ...
  (let* ((dynamic-p (= (elf64-elf-type builder) +elf-type-dyn+))
         ...
         (sh-rela-plt-off (and dynamic-p (strtab-add shstrtab ".rela.plt")))
          ;; Program header table and optional PT_INTERP payload.
         ...
         ;; .text / .rodata / .rodata.str / .eh_frame / .eh_frame_hdr / .data layout.
         ;; Each offset aligns up from the end of the previous section.
         ...
         ;; ET_DYN-only .dynsym / .dynstr / .rela.dyn / .rela.plt / .dynamic layout.
         ;; Every value here is NIL/0 for ET_EXEC, since DYNAMIC-P gates each form.
         ...
         ;; .bss / .symtab / .strtab / .debug_line / .shstrtab layout, then the
         ;; section header table offset (SHOFF) once every payload is placed.
    ...
    (multiple-value-bind (n-sections dynsym-idx dynstr-idx symtab-idx strtab-idx
        ...
        (%elf64-section-header-indices dynamic-p interp-p rodata-str-section-delta)
      (let* (;; PT_LOAD segment sizes, derived from the section offsets above.
    ...
    (with-byte-buffer (out)
      ...
      ;; Section payloads.
      (when interp-p
        (binary-buffer-pad-and-write out interp-offset interp-bytes))
      (binary-buffer-pad-and-write out text-offset text-bytes)
      (binary-buffer-pad-and-write out rodata-offset rodata-bytes)
      (when rodata-str-present-p
        (binary-buffer-pad-and-write out rodata-str-offset rodata-str-bytes))
      (binary-buffer-pad-and-write out eh-frame-offset eh-frame-bytes)
      (binary-buffer-pad-and-write out eh-frame-hdr-offset eh-frame-hdr-bytes)
      (binary-buffer-pad-and-write out data-offset data-bytes)
      (when dynamic-p
        (binary-buffer-pad-and-write out dynsym-offset dynsym-bytes)
        (binary-buffer-pad-and-write out dynstr-offset dynstr-bytes)
        (binary-buffer-pad-and-write out rela-dyn-offset rela-dyn-bytes)
        (binary-buffer-pad-and-write out rela-plt-offset rela-plt-bytes)
        (binary-buffer-pad-and-write out dynamic-offset dynamic-bytes))
      (binary-buffer-pad-and-write out symtab-offset sym-buf)
      (binary-buffer-pad-and-write out strtab-offset strtab-bytes)
      (binary-buffer-pad-and-write out debug-line-offset debug-line-bytes)
      (binary-buffer-pad-and-write out shstrtab-offset shstrtab-bytes)
